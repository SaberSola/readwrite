...
import com.readwrite.common.utils.SpringBeanUtils;
...
public class RedisLimitBootStrap implements ApplicationContextAware {
    ...
    @Autowired
    public RedisLimitBootStrap(final RateLimiterClient rateLimiterClient) {
    ...
    private void start(List<RedisLimiteConfig> redisLimites) {
        for (RedisLimiteConfig redisLimiteConfig : redisLimites) {
            rateLimiterClient.init(redisLimiteConfig.getKey(),redisLimiteConfig.getContext(),
